...
def pcl_callback(pcl_msg):
    ...
    cluster_color_point_list = []

    for j, indices in enumerate(cluster_indices):
        for i, indice in enumerate(indices):
            cluster_color_point_list.append([white_cloud[indice][0],
                                            white_cloud[indice][1],
                                            white_cloud[indice][2],
                                            rgb_to_float(cluster_color[j])])

    cluster_cloud = pcl.PointCloud_PointXYZRGB()
    cluster_cloud.from_list(cluster_color_point_list)
...
def pr2_mover(object_list):
    ...
    for object in object_list:
        ...
        centroids.append([np.asscalar(centroid[0]), np.asscalar(centroid[1]), np.asscalar(centroid[2])])
...
if __name__ == '__main__':
    ...
    pcl_debug_pub = rospy.Publisher("/pcl_debug", PointCloud2, queue_size=1)
